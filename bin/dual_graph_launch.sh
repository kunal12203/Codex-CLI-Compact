...
if [[ "$ASSISTANT" == "codex" ]]; then
  ...
  if [[ "$_CODEX_REG_OK" == "1" ]]; then
    ...
    echo "[$TOOL_LABEL] If it still fails, join Discord: https://discord.com/invite/YwKdQATY2d"
...
PY
  ...
  _TC_PORT=8899
  if grep -q '"token-counter"' "$HOME/.claude.json" 2>/dev/null; then
    if [[ -f "$_TC_PORT_FILE" ]]; then _TC_PORT=$(cat "$_TC_PORT_FILE"); fi
    echo "[$TOOL_LABEL] Token counter already registered -> http://127.0.0.1:$_TC_PORT (global)"
  else
...
if ! kill -0 "$MCP_PID" 2>/dev/null; then
  ...
  echo "[$TOOL_LABEL] Try running $TOOL_LABEL again. If it persists, join Discord: https://discord.com/invite/YwKdQATY2d"
...
if [[ "$ASSISTANT_EXIT" -ne 0 && "$ASSISTANT_EXIT" -ne 130 && "$ASSISTANT_EXIT" -ne 143 ]]; then
  ...
  echo "[$TOOL_LABEL]   4. Join Discord for help: https://discord.com/invite/YwKdQATY2d"
